...
//  FlappyBird
//
//  Created by Hiroki Watanabe on 2021/08/30.
//
import SpriteKit
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let play = SKAction.playSoundFileNamed("sushi", waitForCompletion: true)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            self.run(play)
            itemScoreLabelNode.text = "Sushi:\(itemScore)"    // ←追加
        }else {
            // 壁か地面と衝突した
            print("GameOver")
    ...
    func setupScoreLabel() {
        ...
        itemScoreLabelNode.text = "Sushi:\(itemScore)"
    ...
    func setupItem() {
        ...
        let createItemAnimation = SKAction.run({
            ...
            let random_y = CGFloat.random(in: -random_y_range...random_y_range)
            item.position = CGPoint(x: self.frame.size.width + itemTexture.size().width / 2 ,y: 500+random_y)
            item.zPosition = 0
